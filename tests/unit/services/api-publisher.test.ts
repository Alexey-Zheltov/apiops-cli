...
describe('api-publisher', () => {
  ...
  describe('publishApi', () => {
    ...
    it('should skip WSDL-importer-generated XSD schemas (GUID names) on WSDL import', async () => {
      const client = createMockClient();
      // GUID name is not caught by isAutoGeneratedId (24-hex only), but the
      // WSDL importer recreates XSD schemas, so re-PUTs must be excluded.
      const xsdSchema = {
        type: ResourceType.ApiSchema,
        nameParts: ['soap-api', '4b6fe4d6-dbad-4db0-a6d6-58958e74850d'],
      };
      const store = createMockStore([xsdSchema]);
      store.readResource.mockImplementation(async (_dir: string, descriptor: ResourceDescriptor) => {
        if (descriptor.type === ResourceType.Api) {
          return { name: 'soap-api', properties: { path: 'soap', type: 'soap' } };
        }
        if (descriptor.type === ResourceType.ApiSchema) {
          return {
            name: descriptor.nameParts[1],
            properties: { contentType: 'application/vnd.ms-azure-apim.xsd+xml', document: { value: '<xsd:schema/>' } },
          };
        }
        return null;
      });
      ...
      const apiDescriptor: ResourceDescriptor = { type: ResourceType.Api, nameParts: ['soap-api'] };
      await publishApi(client, store, testContext, apiDescriptor, testConfig);
    ...
    it('should retain GUID-named XSD schemas for non-WSDL spec imports', async () => {
      const client = createMockClient();
      // OpenAPI import does not recreate XSD schemas, so the exclusion must
      // apply only when the imported spec format is wsdl.
      const xsdSchema = {
        type: ResourceType.ApiSchema,
        nameParts: ['rest-api', '4b6fe4d6-dbad-4db0-a6d6-58958e74850d'],
      };
      const store = createMockStore([xsdSchema]);
      store.readResource.mockImplementation(async (_dir: string, descriptor: ResourceDescriptor) => {
        if (descriptor.type === ResourceType.Api) {
