...
async function resolveRootApiPutDescriptor(
  ...
  // Existence must be checked against the deployed (env-mapped) name, and the
  // ;rev=N suffix appended after affixing so suffix mappings don't corrupt it.
  const baseName = getNamePart(descriptor.nameParts, 0);
  const deployedBaseName = config.envMapping
    ? toDeployedName(baseName, ResourceType.Api, config.envMapping)
    : baseName;

  const existing = await client.getResource(context, {
    ...descriptor,
    nameParts: [deployedBaseName, ...descriptor.nameParts.slice(1)],
  });
  if (existing) {
    return descriptor;
  }
