...
export async function planApiPublication(
  ...
  let importSpecificationFormat: string | undefined;
  ...
  if (specificationAllowed) {
    ...
    if (mergedApiJson && specification) {
      ...
      importSpecificationFormat = importSpecification ? specification.format : undefined;
  ...
  if (importSpecification) {
    ...
    // WSDL-import-generated XSD schemas have GUID names (not 24-hex), so the
    // isAutoGeneratedId filter misses them. APIM recreates them on WSDL import
    // and rejects re-PUTs, so exclude schemas with the XSD content type — but
    // only for WSDL imports; other importers do not recreate XSD schemas.
    const nonImporterSchemas = importSpecificationFormat !== 'wsdl'
      ? explicitSchemas
      : (
          await Promise.all(
            explicitSchemas.map(async (descriptor) => {
              const schemaJson = await store.readResource(config.sourceDir, descriptor);
              const contentType = (schemaJson?.properties as Record<string, unknown> | undefined)
                ?.contentType;
              return contentType === 'application/vnd.ms-azure-apim.xsd+xml' ? undefined : descriptor;
            })
          )
        ).filter((descriptor): descriptor is ResourceDescriptor => descriptor !== undefined);
    const filteredExplicitSchemas = config.filter
      ? nonImporterSchemas.filter((descriptor) =>
          shouldIncludeResource(descriptor, effectiveFilter)
        )
      : allowed
        ? nonImporterSchemas.filter((descriptor) =>
