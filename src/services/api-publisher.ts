  ...
  if (importSpecification) {
    const explicitSchemas = allDescriptors.filter(
      (descriptor) =>
        ...
        // Portal-created schemas (13-digit Date.now() names) are recreated by
        // the spec import like other machine-generated schemas; re-PUTs
        // duplicate them on the destination (#274).
        !isPortalGeneratedSchemaId(getNamePart(descriptor.nameParts, 1))
    );
